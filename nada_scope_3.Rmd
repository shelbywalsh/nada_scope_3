...
cheese_category_2 <- all_products %>% 
 filter(grepl('Cheese,|Charmisan|Cream Cheese|Feta|Shreds|Vegan Cheese|Cashew Ricotta', item))
# Questionable whether cream cheese belongs here

# Subtract those words from all_products
all_products <- all_products[!grepl('Cheese,|Charmisan|Cream Cheese|Feta|Shreds|Vegan Cheese|Cashew Ricotta', all_products$item),]
...
# Subtract those words from all_products
all_products <- all_products[!grepl('Bread|Hamburger|Crouton|Loaf', all_products$item),]
...
frozen_category_12 <- all_products %>% 
 filter(grepl('Frozen', item))

# Subtract those words from all_products
all_products <- all_products[!grepl('Frozen', all_products$item),]
...
dairy_category_15 <- all_products %>% 
 filter(grepl('Milk|Mylk|Butter, Salted|Butter, Vegan|Cultured Butter', item))

# Subtract those words from all_products
all_products <- all_products[!grepl('Milk|Mylk|Butter, Salted|Butter, Vegan|Cultured Butter', all_products$item),]
...
oilseed_category_16 <- all_products %>% 
 filter(grepl('Oil|Tofu|Tempeh|Tempea|Soy Protein|Pea Protein', item))
...
oilseed_category_17 <- all_products %>% 
 filter(grepl('Lentil|Peas|Soybeans|, Raw|Cashews, Whole', item))
```
...
```{r}
snack_category_16 <- all_products %>% 
 filter(grepl('Cashew|Almond|Peanut|Chip', item))
# Avoiding capturing "butternut"
...
(11) Fruit and tree nut farming
```{r}
category_11 <- all_products %>% 
 filter(grepl('Raw', item))
...
syrup_category_13 <- all_products %>% 
 filter(grepl('Lentil|Peas|Soybeans', item))
```
