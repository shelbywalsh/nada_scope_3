...
emissions_transport <- purchaseorder_distance %>% 
  ...
  mutate(kg_CO2pergal = 8.78) %>%
  mutate(kgco2e = sum_distance/10.46*kg_CO2pergal) %>% # Calculate transportation emissions for CO2 for each vendor
  mutate(total_kgco2e = sum(kgco2e)) %>%  # Sum total transportation emissions for Nada
  mutate(percent_contribution = kgco2e/total_kgco2e*100) %>% # See percent contribution to emissions by supplier
  mutate(cumulative_pct_emissions = cumsum(percent_contribution)) # See cumulative percent contribution
```

# Filter (for fun) purchase order distance to eliminate suppliers >50km, then re-calculate total emissions
```{r}
local_distance <- purchaseorder_distance %>% 
...
Using 'delivery truck' fuel economy (6.5mpg) from here:
https://afdc.energy.gov/data/10310 

And mobile combustion CO2 'motor gasoline' emission factor (8.78kgCO2/gallon) from EPA:  https://www.epa.gov/sites/production/files/2018-03/documents/emission-factors_mar_2018_0.pdf

6.5miles/gallon = 10.46 km / gallon

x km / 10.46km per gallon = left with gallons

Then multiply gallons by:
From EPA Motor Gasoline
8.78 kg CO2 per gallon 
